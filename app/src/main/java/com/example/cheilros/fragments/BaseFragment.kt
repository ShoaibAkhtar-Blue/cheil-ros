...
open class BaseFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        if(fragmentLabel == "dashboard"){
            configureToolbar(settingData.filter { it.fixedLabelName == "Dashbord_Title" }.get(0).labelName)
        }else if(fragmentLabel == "journeyplan"){
            configureToolbar(settingData.filter { it.fixedLabelName == "JourneyPlan_Title" }.get(0).labelName, true)
        }else if(fragmentLabel == "mycoverage"){
            configureToolbar(settingData.filter { it.fixedLabelName == "StoreList_Title" }.get(0).labelName, true, true)
        }
